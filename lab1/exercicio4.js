...
function initBuffers() {
	...
	sphereLong = buildSphereLong(15, 30);
...
function buildSphere(np, np2){
	...
	vPos.intervalSize = vPos.numItems / vPos.numIntervals;
...
// ********************************************************
// ********************************************************
function buildSphereLong(np, np2){
	var vPos = new Array;
	var angle = np *  (Math.PI / 180.0);
	var angle2 = np2 *  (Math.PI / 180.0);
	var r = 1;
...
function drawScene() {
	...
	setMatrixUniforms();
	for(var interval = 0; interval < sphere.numIntervals; interval++)
		gl.drawArrays(gl.LINE_STRIP, sphere.intervalSize*interval, sphere.intervalSize);

	gl.bindBuffer(gl.ARRAY_BUFFER, sphereVerticesLongBuffer);
	gl.vertexAttribPointer(vertexPositionAttribute, 3, gl.FLOAT, false, 0, 0);
	for(var interval = 0; interval < sphereLong.numIntervals; interval++)
		gl.drawArrays(gl.LINE_STRIP, sphereLong.intervalSize*interval, sphereLong.intervalSize);
